...
export function AppSettingsProvider({ children }) {
  ...
  const accent = useMemo(() => {
    ...
    const hover = mixHex(option.hex, 0.1, foreground === '#ffffff' ? 'black' : 'white')
    const soft = mixHex(option.hex, 0.85, 'white')
    const ring = mixHex(option.hex, 0.7, 'white')
    const shadow = mixHex(option.hex, 0.3, 'black')
